...
function Detail({ onClick, data }) {
    ...
    const onSubmit = async (data) => {
        if (Object.keys(logined).length === 0) {
            setLogin(true);
        } else {
            if (data.number) {
                var getAPICart = await cartService.get(logined.id);
                var quantityPrev = 0;
                if (getAPICart != null) {
                ...
                await cartService.update(`${logined.id}/${dataDetail.id}`, { ...dataDetail, quantity: parseInt(data.number) + parseInt(quantityPrev) });
                onClick();
            }
        }
    };
